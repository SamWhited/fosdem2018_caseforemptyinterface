...
\documentclass[xelatex,aspectratio=43]{beamer}
...
\author[]{%
	Sam Whited\\*%
	{\tiny XMPP: \texttt{\href{xmpp:sam@samwhited.com}{sam@samwhited.com}}}%
}
\date{2017--02--03}
...
\begin{frame}
	...
	\texttt{\color{MidGreen}// }\textit{%
		deadlines, cancelation signals, and other \only<1>{request-scoped}\only<2>{\strong{request-scoped}}}\\
	\texttt{\color{MidGreen}// }\textit{%		
		\only<1>{values}\only<2>{\strong{values}} across API boundaries and
		between processes.}
\end{frame}
...
\begin{frame}
	...
	{\color{AshGray}%
		...
		\tikz[overlay,remember picture] {
			...
			west) -- node [above=1.75em] {\small email \& xmpp} (end.north east) ;
