...
\begin{frame}
	...
	\begin{enumerate}
		\item<4-> 1\hspace*{10.5em}→ <int>1</int>
		\item<5-> ``To sit in solemn silence" → <string>To sit in solemn silence</string>
		\item<6-> \type{xml.Marshaler}\hspace*{4.2em}→ \method{T.MarshalXML(e, start)}
		\item<7-> struct\{ Name string \}\hspace*{1.8em}→ ???\only<8->{\ (reflection!)}
	\end{enumerate}
\end{frame}
